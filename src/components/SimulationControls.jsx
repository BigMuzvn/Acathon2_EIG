...
import { Calendar, MapPin, Gauge, Sparkles } from 'lucide-react';
  ...
  return (
    <section className="glass-card p-6 md:p-8 mb-8 border border-white/10 shadow-2xl">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {presets.map((p, idx) => (
            <button
              ...
              className="px-3 py-1.5 bg-slate-800/90 hover:bg-blue-600/30 hover:border-blue-500/50 text-slate-200 text-xs rounded-xl border border-white/10 transition-all font-medium flex items-center gap-1.5 shadow-sm"
            >
              <span>{p.icon}</span>
              <span>{p.label}</span>
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Kilométrage Annuel */}
        <div className="p-5 bg-slate-900/80 rounded-2xl border border-white/10 flex flex-col justify-between shadow-inner">
          <div className="flex items-center justify-between gap-2 mb-3">
            <label className="text-sm font-bold text-slate-200 flex items-center gap-2">
              <Gauge className="w-4 h-4 text-blue-400 shrink-0" /> Kilométrage annuel
            </label>
            <span className="text-sm font-bold text-blue-400 font-mono bg-blue-500/15 px-3 py-1 rounded-lg border border-blue-500/30 shrink-0">
        ...
        <div className="p-5 bg-slate-900/80 rounded-2xl border border-white/10 flex flex-col justify-between shadow-inner">
          <div className="flex items-center justify-between gap-2 mb-3">
            <label className="text-sm font-bold text-slate-200 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-indigo-400 shrink-0" /> Durée de possession
            </label>
            <span className="text-sm font-bold text-indigo-400 font-mono bg-indigo-500/15 px-3 py-1 rounded-lg border border-indigo-500/30 shrink-0">
        ...
        <div className="p-5 bg-slate-900/80 rounded-2xl border border-white/10 flex flex-col justify-between shadow-inner">
          <div className="flex items-center justify-between gap-2 mb-3">
            <label className="text-sm font-bold text-slate-200 flex items-center gap-2">
              <MapPin className="w-4 h-4 text-emerald-400 shrink-0" /> Région / Prix Énergie
            </label>
            <span className="text-xs text-emerald-400 font-mono font-semibold">Tarif local</span>
          </div>
